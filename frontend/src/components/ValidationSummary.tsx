      ...
      {result.missingRequiredColumns.length > 0 && (
        <Card className="border-red-200 bg-red-50">
          <CardContent className="flex items-start gap-3 p-4">
            ...
            <div>
              <p className="font-medium text-red-800">Colunas obrigatórias não mapeadas</p>
      ...
      <div className="grid gap-4 sm:grid-cols-3">
        <Card className="border-green-200 bg-green-50">
          <CardContent className="flex items-center gap-3 p-4">
            ...
            <div>
              <p className="text-2xl font-bold text-green-800">{formatNumber(result.validCount)}</p>
        ...
        <Card className="border-amber-200 bg-amber-50">
          <CardContent className="flex items-center gap-3 p-4">
            ...
            <div>
              <p className="text-2xl font-bold text-amber-800">{formatNumber(result.warningCount)}</p>
              <p className="text-sm text-amber-700">registros com alertas</p>
            </div>
          </CardContent>
        </Card>
        ...
        <Card className="border-red-200 bg-red-50">
          <CardContent className="flex items-center gap-3 p-4">
            ...
            <div>
              <p className="text-2xl font-bold text-red-800">{formatNumber(result.invalidCount)}</p>
              <p className="text-sm text-red-700">registros inválidos</p>
            </div>
          </CardContent>
        </Card>
      </div>
